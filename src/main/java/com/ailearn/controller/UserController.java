...
import java.util.Map; // Java标准库类，键值对映射接口，用于存储和返回动态数据
...
public class UserController { // 用户认证控制器类定义
    ...
    public Result<Map<String, Object>> refreshToken( // 刷新Token接口方法定义
            @Parameter(description = "刷新Token请求参数", required = true) // OpenAPI注解，描述刷新Token请求参数
            @Valid @RequestBody RefreshTokenRequest request) { // @Valid启用参数校验，@RequestBody将请求体解析为RefreshTokenRequest对象
        log.info("收到Token刷新请求"); // 记录INFO级别日志，输出收到Token刷新请求
        String newAccessToken = userService.refreshToken(request.getRefreshToken()); // 调用用户服务的刷新Token方法，传入refreshToken获取新的accessToken
        Map<String, Object> data = Map.of("accessToken", newAccessToken); // 使用Map.of创建不可变Map，存储新的accessToken
        return Result.success(data); // 返回成功响应，包装新的accessToken
    } // refreshToken方法结束
